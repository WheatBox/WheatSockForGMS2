...
class MyAPI {
    ...
    public:
        ...
        int html_bufRemoveHead(char * buf, int len);
...
const char * MyAPI::getFile_html(const char * getFilename, const char * destFilename) {
    ...
    do {
        ...
        if(packsnum == 1 && getFile_htmlRemoveHead == true) {
            recvlen_use = html_bufRemoveHead(recv_buf, recvlen);
...
int MyAPI::html_bufRemoveHead(char * buf, int len) { // 移除接收到的响应头
